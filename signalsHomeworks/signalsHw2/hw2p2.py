...
def pitchShift(sample, frame_size, hopA, step):
    ...
    hopOut = np.round(step_size*hopA
                      )  # hopOut will be used in synthesis stage.
    ...
    for i in range(numFramesInput):
        ...
        outputMagnitute = magFrame
        outputFrame = np.real(np.fft.ifft(np.multiply(outputMagnitute, np.exp(1j*CumulativePhase))))
        outputY[i,:] = np.multiply(outputFrame, wn) / np.sqrt(frame_size/(2*hopA))
...
pitchedAudio  = pitchShift(averageChannel, 1024, 256, 6)
#time = 10
#x_axis = np.linspace(0, time, time*sample_rate, endpoint=False)
#plt.plot(x_axis,y)
#plt.show()
speedIncreased = np.interp (np.arange(0,np.size(pitchedAudio,0)-1,2**(1/2)),np.arange(0,np.size(pitchedAudio,0)), pitchedAudio)
speedIncreased = np.asarray(speedIncreased, dtype=np.int16)
wav.write('nightcore.wav', sample_rate,speedIncreased)
time = 10
x_axis = np.linspace(0, time, time*sample_rate, endpoint=False)
plt.plot(x_axis,speedIncreased[:time*sample_rate])
plt.show()